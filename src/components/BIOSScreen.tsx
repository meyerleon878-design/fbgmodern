...
const BIOSScreen = ({ onExit, onFactoryReset }: BIOSScreenProps) => {
  ...
  const tabContents: Record<number, { label: string; value: string; action?: () => void }[]> = {
    ...
    3: [
      ...
      { label: '>>> Factory Reset', value: '[Press Enter]', action: () => setShowResetConfirm(true) },
    ],
    4: [
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'ArrowRight') {
      ...
      setSelectedItem(prev => Math.min(prev + 1, currentItems.length - 1));
    } else if (e.key === 'ArrowUp') {
      setSelectedItem(prev => Math.max(prev - 1, 0));
    } else if (e.key === 'Enter') {
      const item = currentItems[selectedItem];
      ...
      <div className="min-h-screen bg-[#0000aa] text-white font-mono flex flex-col items-center justify-center">
        ...
        <div className="w-96">
          ...
          <div className="w-full h-6 bg-[#000066] border border-gray-400">
            <motion.div
              className="h-full bg-[#00aa00]"
              style={{ width: `${resetProgress}%` }}
            />
          </div>
          <div className="text-sm mt-2 text-center">{Math.floor(resetProgress)}%</div>
          <div className="text-xs mt-4 text-gray-300 text-center">
  ...
  return (
    <div
      className="min-h-screen bg-[#0000aa] text-white font-mono p-4 focus:outline-none"
      tabIndex={0}
          ...
          <div className="bg-[#0000aa] border-2 border-white p-6 max-w-md">
            ...
            <div className="flex gap-4">
              <button
                ...
                className="px-4 py-2 bg-[#aa0000] text-white border border-white hover:bg-[#ff0000]"
              >
                YES - Factory Reset
              </button>
              <button
